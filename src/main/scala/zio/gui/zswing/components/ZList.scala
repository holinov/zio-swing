...
object ZList {
  ...
  object ObservableListModel {
    private class SeqRefListModelImpl[T](val stateRef: Ref[IndexedSeq[T]],
                                         rt: Runtime[Any])
  ...
  def build(items: String*): Task[ZList[String]] =
    ObservableListModel
      ...
      .map(mdl => new ZList(new JList(mdl), mdl))

  def build[T](mdl: ListModel[T]): Task[ZList[T]] =
    Task(new ZList(new JList(mdl), mdl))

  def buildCh[T](mdl: ListModel[T],
                 onSelected: T => Task[Unit]): Task[ZList[T]] =
    Task(new ZList(new JList(mdl), mdl)) tap { zl =>
      zl.onSimpleSelect(onSelected)
    }
}

final class ZList[T](val component: JList[T], mdl: ListModel[T])
    extends ZComponent[JList[T]] {
  ...
  private def addSelectionListener(rt: Runtime[Any],
                                   act: (Int, Int) => Task[Unit]): Task[Unit] =
    Task(
      component.addListSelectionListener(
        (e: ListSelectionEvent) =>
          rt.unsafeRun(act(e.getFirstIndex, e.getLastIndex))
      )
    ).unit
  ...
  def onSimpleSelect(act: T => Task[Unit]): Task[Unit] =
    for {
      ...
      _ <- addSelectionListener(
        rt,
        (f, t) =>
          selected.flatMap(s => selectedQueue.offer((f, t, s)).fork.unit)
      )
      _ <- {
